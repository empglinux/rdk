	...
	size_t size, loff_t *ppos)
{
	...
	{
		to_user_event = fifo_que[que_head++];
		que_head %= MAX_NUM;
		dbg_print("event = %d", to_user_event);
		if ( que_head != que_tail )
			flag = 1;
	}
...
{
	dbg_print("event = %d", event);
	if ( que_head != ( que_tail + 1 ) % MAX_NUM )
	{
		fifo_que[que_tail++] = event;
		que_tail %= MAX_NUM;
	}
	flag = 1;
	wake_up_interruptible(&outq);
...
#else
	vkd_entry = proc_create_data(CMPC_VKD_NAME, S_IRUGO, acpi_root_dir, &vkd_fops, vkd_dev);
#endif
...
static int cmpc_vkd_remove(struct acpi_device *device) {
	dbg_print("----->\n");
